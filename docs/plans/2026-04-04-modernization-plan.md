...
## Step 10: Eliminate unused_assignments suppressions

**File**: `src/lib.rs`

### 10a. Implementation

**Site 1 — `follow()` (~line 346)**: Remove `#[allow(unused_assignments)]` and `let mut to = 0;`. Restructure as:
```rust
...
Note: `to` must remain `mut` in the else branch since `resolve` may reassign it. Use `let mut to;` with no initial value.

**Site 2 — `find()` (~line 371)**: Remove `#[allow(unused_assignments)]` and change `let mut to: usize = 0;` to declare `to` inside the loop:
```rust
while pos < key.len() {
    // ...
...
**Site 3 — `erase__()` (~line 447)**: Remove `#[allow(unused_assignments)]`. Move `has_sibling` declaration inside the loop:
```rust
loop {
...
**Site 4 — `next()` (~line 559)**: Remove `#[allow(unused_assignments)]` and `let mut c: u8 = 0;`. Use cfg-conditional binding:
```rust
#[cfg(feature = "reduced-trie")]
...
Note: `c` is later reassigned in the while loop body, so it still needs to be `let mut c`.
...
### 10c. Commit
```bash
git commit -m "Eliminate unused_assignments suppressions with cleaner variable declarations"
```
...
Remove `#[allow(clippy::cast_lossless)]` from the `impl Cedar` block (line 240).

Run `cargo clippy` to identify which `as i32` casts are flagged as lossless (these are `u8 as i32` widening casts). Convert each flagged site from `x as i32` to `i32::from(x)`. Examples:
- `base ^ (label as i32)` → `base ^ i32::from(label)`
- `(c as i32)` → `i32::from(c)` where c is u8

Do NOT convert narrowing or same-width casts (e.g., `usize as i32`, `i32 as usize`).
...
5. Fix the example code — add `.unwrap()` after `common_prefix_search()` calls (the README examples are missing this but the doc examples in lib.rs have it):
   ```rust
...
- `actions-rs/tarpaulin@v0.1` → `run: cargo install cargo-tarpaulin && cargo tarpaulin --all-features --out xml`
...
| 4 | Step 6 | No | Dead code cleanup, depends on Step 5's removal of pop_sibling annotation |
| 5 | Steps 7-9 | Yes | Option semantics, Default impl, next_until_none are independent |
| 6 | Step 10 | No | unused_assignments cleanup, after lint suppressions are addressed |
| 7 | Step 11 | No | Typos + Block Default, minor cleanup |
| 8 | Step 12 | No | Cast cleanup — large mechanical change, separate commit |
| 9 | Steps 13-14 | Yes | README and doc comment are independent |
